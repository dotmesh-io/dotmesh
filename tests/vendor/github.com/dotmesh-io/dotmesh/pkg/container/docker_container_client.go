...
func New(options *Options) (*DockerClient, error) {
	...
	return &DockerClient{
		containerMountPrefix:  options.ContainerMountPrefix,
		client:                client,
		containersStopped:     make(map[string]map[string]string),
		containerMountDirLock: options.ContainerMountDirLock,
	}, nil
}
...
func (d *DockerClient) findDotRoot(path string) string {
	if !strings.HasPrefix(path, d.containerMountPrefix+"/") {
		log.Printf("[findDotRoot] Container mount path %v doesn't start with %v/", path, d.containerMountPrefix)
		return path
	}
	subpath := strings.TrimPrefix(path, d.containerMountPrefix+"/")
	parts := strings.Split(subpath, "/")
	switch len(parts) {
	case 2: // no subdot
		return path
	case 3: // subdot
		return d.containerMountPrefix + "/" + parts[0] + "/" + parts[1]
	default:
...
func (d *DockerClient) relatedFilesystems(container *docker.Container) ([]string, error) {
	...
	for _, mount := range container.Mounts {
		...
		target, err := os.Readlink(d.findDotRoot(mount.Source))
		if err != nil {
		...
		for _, mount := range container.Mounts {
			if mount.Driver == "dm" {
				...
				if baseDotName(mount.Name) == volumeName {
					...
					// switcharoo.
					err = func() error {
						d.containerMountDirLock.Lock()
						defer d.containerMountDirLock.Unlock()
