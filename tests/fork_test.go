...
func TestForks(t *testing.T) {
	...
	t.Run("CreateThenFork", func(t *testing.T) {
		...
		// Alice deletes the fork
		citools.RunOnNode(t, node1Name, "dm remote switch alice")
		citools.RunOnNode(t, node1Name, "dm dot delete -f alice/"+fsname2)
	})

	t.Run("DeleteThenForkAgain", func(t *testing.T) {
		fsname := citools.UniqName()
		...
		// Alice deletes her fork (didn't like the color)
		citools.RunOnNode(t, node1Name, "dm remote switch alice")
		citools.RunOnNode(t, node1Name, "dm dot delete -f alice/"+fsname2)
		...
		// Alice looks for it
		citools.RunOnNode(t, node1Name, "dm remote switch alice")
		secondOutput := citools.OutputFromRunOnNode(t, node1Name, "dm list")
		if !strings.Contains(secondOutput, fsname2) {
			t.Errorf("Did not find dot %s in output. Got: %s", fsname2, secondOutput)
		}
		citools.RunOnNode(t, node1Name, "dm switch alice/"+fsname2)
		secondOutput = citools.OutputFromRunOnNode(t, node1Name, "dm log")
		if !strings.Contains(secondOutput, "Nice Commit") {
			t.Errorf("Did not find commit in output. Expected '%s', got: '%s'", "Nice Commit", secondOutput)
		}
	})
}
