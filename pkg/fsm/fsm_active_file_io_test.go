...
func TestFsmActiveCleanDuringUploadDir(t *testing.T) {
	...
	// now, overwrite it
	f2, err := os.Open("./testdata/archived-second-file_tar")
	if err != nil {
		t.Fatalf("failed to open test file: %s", err)
	}
	secondFile := &types.InputFile{
	...
	contents, err := ioutil.ReadFile(outputDir + "/extracted/second-file.txt")
	if err != nil {
		t.Errorf("failed to read saved contents: %s", err)
		return
	}
