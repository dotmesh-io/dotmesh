...
const DOTMESH_DOCKER_IMAGE = "quay.io/dotmesh/dotmesh-server:latest"
...
another.`,
	...
	cmd.PersistentFlags().StringVar(
		&dotmeshDockerImage, "image", DOTMESH_DOCKER_IMAGE,
...
func NewCmdClusterUpgrade(out io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "upgrade",
		Short: "Upgrade the Dotmesh server on this node to the client version (Use 'dm version' to check)",
		Run: func(cmd *cobra.Command, args []string) {
		...
		connected = func() bool {
			...
			e := func() {
				if try == 4*30 { // 30 seconds (250ms per try)
					fmt.Printf(
						"\nUnable to connect to dotmesh server after 30s, " +
